...
def evaluate_models_allstates_plot(cma_results,train_len,model_names=['EMM-GPD','GMM'],quantiles=[0.9, 0.99, 0.999, 0.9999, 0.99999],xsize=3,markers=['s','o','v','*','x'],loglog=False, ylim_ll=[1e-4,1e0],ylim=[0,0.4]):
    ...
    for m in range(n_plots):
        ...
        for i in range(len(quantiles)):
            ...
            path = axes[m].scatter(results[:,-1],abs(results[:,xsize+i]),c=colors_arr,marker=markers[i])
        ...
        axes[m].legend(handles=leg_custs)
        axes[m].grid()
        axes[m].set_title(model_names[m])

        if loglog is False:
            axes[m].set_ylim(ylim)
            axes[m].set_ylabel('Standardized error of quantiles')
            axes[m].set_xlabel('P(x)')
        else:
            axes[m].set_ylim(ylim_ll)
            ax0t.set_xscale('log')
            ax0t.set_yscale('log') 
        ...
        ax0t.set_xlim(axes[m].get_xlim())
        top_xticks = axes[m].get_xticks()
        ax0t.set_xticks(top_xticks)
        ax0t.set_xbound(axes[m].get_xbound())
        ax0t.set_xticklabels([math.floor(x * train_len) for x in axes[m].get_xticks()])
