...
class GPDTailDistribution():
  ...
  def prob(self, values, norm_factor):
    ...
    self.tres = tf.multiply(norm_factor,tf.multiply(tf.divide(1,self.sigmas),tf.pow(1+tf.multiply(self.zais,tf.divide(self.values_sq-self.us,self.sigmas)),tf.divide(-self.zais-1.00,self.zais))))
  ...
  def tail_prob(self, values, norm_factor):
    ...
    self.sigmas = tf.squeeze(self.tail_scale)
    # if zais are not zero
    self.ctres = tf.multiply(norm_factor,tf.multiply(norm_factor,tf.pow(1+tf.multiply(self.cvalues_sq-self.cus,tf.divide(self.zais,self.sigmas)),tf.divide(-1,self.zais))))
...
class GPDExtremeValueMixtureDensityNetwork(BaseNNMixtureEstimator):
